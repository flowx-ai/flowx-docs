...
The complete list of active **process instances** is visible from the FLOWX Designer. The platform provides robust search and filtering capabilities to help you find specific instances:

* **Search by ID**: Quickly locate process instances using their unique identifier
* **Filter by process definition**: Narrow down instances by selecting specific process definition names
* **Status filtering**: Filter instances by their current status (Running, Completed, Failed)
* **Date range**: Find instances within specific date ranges
* **Advanced search**: Combine multiple search criteria for precise results
* **Real-time updates**: Search results update automatically as new instances are created or existing ones change status

You can also view detailed information about each process instance, including its current status, data, and execution history.
