...
The Integration Management interface includes a search functionality that helps you quickly find specific integrations:

* Use the search bar at the top of the integration list to search by integration name or system name
* Filter integrations based on:
  * Integration status (active/inactive)
  * System name
  * Scenarios used
  * Environment

The search is performed in real-time as you type, making it easier to locate the integration you need among multiple entries.
...
[Configuring access rights for Integration Management](/3.x/docs/platform-deep-dive/core-extensions/integration-management/configuring-access-rights-for-intgr-mngmnt)
...
* Node → Node config → **Data stream topics** (the topic name where the [process engine](/3.x/setup-guides/flowx-engine-setup-guide) listens for the response)
