...
description: Allocation rules are meant to define when tasks should be auto-assigned to users when they reach a [swimlane](../../../user-roles-management/swimlanes) that has a specific role configured (for example, specific tasks will be assigned for the _front office_ and specific tasks for the _back office_ only).
---
...
5.  **Option 2**: Allocate only users you choose from the drop-down list. 

When selecting specific users, you can utilize the search functionality to quickly find the desired users:
- Type any part of the user's name in the search box
- The search is case-insensitive and matches partial names
- Results update in real-time as you type
- You can search by first name, last name, or both
- The search box supports backspace and clear functionality for easier editing
...
<Info>
Users with out-of-office status will be skipped by automatic allocation. More information about out-of-office feature, [here](/3.x/docs/platform-deep-dive/plugins/custom-plugins/task-management/using-out-of-office-records).
</Info>
