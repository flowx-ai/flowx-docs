...
To begin the document generation process, HTML templates must be created or imported. Utilize the [**WYSIWYG**](/3.x/docs/platform-deep-dive/plugins/wysiwyg) editor accessible through **FLOWX Designer → Plugins → Document templates**.

Learn more about managing HTML templates:

[Managing HTML Templates](./managing-html-templates)
...
- **language**: Should match the language set on the template (a template can be created for multiple languages as long as they are defined in the system, see [**Languages**](/3.x/docs/platform-deep-dive/core-extensions/content-management/languages) section for more information)
