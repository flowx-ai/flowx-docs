...
<Info>
In the example below, we use a `fileId` generated for a document using [<u>**Uploading a New Document**</u>](/3.x/docs/platform-deep-dive/plugins/custom-plugins/documents-plugin/uploading-a-new-document) scenario.
...
1. Create a process that includes a [**Send Message Task (Kafka)**](../../../../building-blocks/node/message-send-received-task-node) node and [**Message Event Receive (Kafka)**](/3.x/docs/building-blocks/node/message-send-received-task-node#configuring-a-message-receive-task-node) node:
