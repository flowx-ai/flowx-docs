---
title: "Message Catch Boundary Events in BPMN"
description: "Learn about Message Catch Boundary Events in BPMN processes - both interrupting and non-interrupting types. Understand how these boundary events handle message flows, event handling, and process interruption in workflow automation."
---

**Why is it important?** It empowers processes to actively listen for and capture designated messages during the execution of associated user tasks. Message catch boundary events are essential components for handling message-based interruptions and parallel message processing in business processes.
...
![Message catch boundary events with multiple events](https://s3.eu-west-1.amazonaws.com/docx.flowx.ai/release34/message_catch_boundary_multiple.png)

Message Catch Boundary Events can be categorized by their behavior, resulting in two main classifications:

* [**Interrupting Message Catch Events**](#message-catch-interrupting-event)  
* [**Non-interrupting Message Catch Events**](#message-catch-non-interrupting-event)

## Message catch interrupting event

<Frame>

![Interrupting message catch boundary event](https://s3.eu-west-1.amazonaws.com/docx.flowx.ai/building-blocks/node/message_catch_interrupting_event.png#center)
...
- **Use Cases:**
  ...
  - Ideal for handling emergency situations or priority changes in workflow
...
- **Example:**
  ...
  - Customer escalation handling during an ongoing process
  - Emergency stop scenarios in manufacturing processes
...
![Non-interrupting message catch boundary event](https://s3.eu-west-1.amazonaws.com/docx.flowx.ai/building-blocks/node/%20message_catch_non_interrupting.png#center)
...
- **Use Cases:**
  ...
  - Perfect for parallel processing and monitoring scenarios

- **Example:**
  - A user task continues its execution while simultaneously capturing and processing non-critical messages
  - Status updates during long-running processes
  - Logging or audit trail creation during process execution
...
![Boundary events illustration](https://s3.eu-west-1.amazonaws.com/docx.flowx.ai/release34/boundary_multiple.png)
...
- Message Processing</fileContent>
</invoke>