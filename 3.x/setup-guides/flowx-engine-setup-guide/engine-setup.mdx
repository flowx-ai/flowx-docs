...
* `SECURITY_OAUTH2_SERVICE_ACCOUNT_ADMIN_CLIENT_ID`: The process engine requires a process engine service account to make direct calls to the Keycloak API. This setting specifies the service account that is essential so the of [**Start Catch Event**](/3.x/docs/building-blocks/node/message-events/message-catch-start-event) node is possible. Ensure that you provide the correct client ID for this service account.
...
📄 [**Task management plugin**](../../docs/platform-deep-dive/plugins/custom-plugins/task-management/task-management-overview)
</Info>


#### Topics related to the scheduler extension

[Scheduler](../../docs/platform-deep-dive/core-extensions/scheduler)
...
<Card title="Using the scheduler" href="../../docs/platform-deep-dive/core-extensions/scheduler#using-the-scheduler" icon="file">
</Card>
...
Below you can find a configuration .yaml to use [scheduler](../../docs/platform-deep-dive/core-extensions/scheduler) service together with FLOWX.AI Engine:
...
Install the latest build version of **process-engine** corresponding to your platfom release version.

</Step>
<Step title ="Configuration">

Enable partitioning and set intervals in process engine configuration with latest environment variables:

[See below](#configuring-partitioning-and-archiving)

Define the appropriate cron expression for the archiving scheduler.

</Step>

<Step title="Operation">

Monitor and manage partitioning and archiving processes.

Review logs for partitioning actions and SQL executions.
</Step>
</Steps>
...
This section contains environment variables that control the settings for data partitioning and archiving and also fo the archiving scheduler. These settings determine how data is segmented, retained, and managed, including compression.
