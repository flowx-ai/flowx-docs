...
2. **Libraries**: Libraries encapsulate reusable resources and offer a structured way to manage elements like **enumerations**, **substitution tags**, and **CMS components** shared across applications.
...
1. **CMS Components**: Custom CMS enumerations, substitution tags, and media items unique to the application.
2. **Application Builds**: Builds represent finalized versions of the application, including all associated metadata and linked library versions.
3. **Rendering Parameters**: Application-specific rendering settings like `applicationUuid`, `locale`, `language`, and `process parameters`.
...
- **Configuration Overwrites**: Upper environment values replace development defaults, and these can be managed through environment variables, enabling flexibility without direct development environment access.
