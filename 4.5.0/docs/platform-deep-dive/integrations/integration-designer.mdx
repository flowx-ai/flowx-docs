...
1. **Integrate the workflow** into a BPMN process:
   - Open the process diagram and include a **User Task**.
   - Add a **Start Integration Workflow** action to trigger the workflow upon user submission.
...
**Receive Workflow Output**:
   - Use a **Receive Message Task** to capture workflow outputs like status or returned data.
