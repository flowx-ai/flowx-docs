...
2. **Configure Actions for the Task**:
   - In the task properties, add a **Start Integration Workflow** action.
...
3. **Map the Payload**:
   - Set up a payload containing the keys and values to be sent to the workflow. This typically includes data fields defined in the data model.
   - The payload enables dynamic inputs to flow from the process into the workflow based on user actions.


</Step>

<Step title="Send Data from Process to Workflow">

1. **Define Input Data**:
   - In the **Start Integration Workflow** action, ensure that input parameters from the process are mapped to the workflow's **Start Node**.
   - Use form fields in the UI Designer to gather input data and ensure they correspond to the expected parameters in the workflow.

2. **Validate Input Mapping**:
   - Verify that the data from the BPMN process is accurately passed into the workflow's input fields. Incorrect or missing mappings can cause errors during workflow execution.
...
1. **Add a Receive Message Task**:
   ...
   - This task captures the workflow’s output data, such as processing status or results.

2. **Set Up a Data Stream Topic**:
   - Define a **Data Stream Topic** in the Receive Message Task that aligns with the workflow's output.
...
Scenario: Integrating with Airtable to manage credit data using REST API.

1. Fetch Existing Records:

* Use a GET REST Endpoint to retrieve a list of records.
* If the request fails, end the workflow with a failure status.

2. Create New Records:

* Use a POST REST Endpoint to add a new record to Airtable.
* Automatically utilize parameters and body data configured at the system level.

3. Check Credit Score:

* Add a Condition Node to evaluate the credit score. If it is below 300, generate a warning message.
* End the workflow with a success or failure status depending on the outcome.
