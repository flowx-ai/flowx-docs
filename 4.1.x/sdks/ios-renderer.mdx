...
In Xcode, click `File` -> `Add Packages...`, enter FlowX repo's URL `https://github.com/flowx-ai-external/flowx-ios-renderer`.
...
<Tip>
It is recommended to call the `FXConfig` configuration method at app launch.<br/>Call the FXSessionConfig configure method after the user logs in and a valid user session is available. 
</Tip>
