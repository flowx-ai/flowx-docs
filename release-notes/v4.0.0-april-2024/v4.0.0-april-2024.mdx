...
- **AuthToken Management**: The ui-sdk no longer relies on the authToken being stored in LOCAL_STORAGE. 
- The **Subprocess Run** node is now called **Call activity** node.
- Changes in the way that  process instance data is handled inside business rules.  
...
- New environment variables to prevent log flooding.

<Check>
Do not to forget to consult the [**migration guide**](migrating-from-v3.x.mdx#) for more information.
</Check>
