...
* Fixed issue which caused conflicts including notification and document templates to fail.
* Fixed issue when merge failed in case of a deleted configuration parameter and updated on the branch.
* Fixed issue which resulted in duplicate substitution tag keys to appear in merged version.
* Fixed issue when merged failed due to the same dependency being added on both source and target branch.
* Fixed issues on workflow conflicts.
