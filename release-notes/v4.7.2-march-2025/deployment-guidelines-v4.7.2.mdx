...
<Info>
When `KAFKA_DEFAULT_FX_CONTEXT` is set, any message without an explicit fxContext header will automatically use this value, ensuring proper message routing even for services that don't explicitly set context headers.
</Info>