...
<Warning>
When upgrading from 4.0 version or an earlier version, note that all MD5 hashes for Liquibase scripts in the `databasechangelog` table will be automatically modified. As a result, attempting to roll back to the previous engine version may lead to startup issues.
