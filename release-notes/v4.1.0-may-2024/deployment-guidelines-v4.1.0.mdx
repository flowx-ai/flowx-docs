...
<Info>
Take in consideration the following backup solution in case of roll back.
- Steps for Creating and Utilizing a Backup for `databasechangelog`

1. **Create a backup of the table:**
   - Duplicate the `databasechangelog` table.
   - Rename the duplicate table to `databasechangelog_backup`.
...
* Rename the `databasechangelog_backup` table back to `databasechangelog`.

```sql
Copy code
DROP TABLE databasechangelog;
ALTER TABLE databasechangelog_backup RENAME TO databasechangelog;
```

4. If the upgrade is successful:

* Drop the `databasechangelog_backup` table.
...
* `SPRING_ELASTICSEARCH_REST_URIS`: Only the value changes for this environment variable for microservices that use Elasticsearch (FlowX Admin, FlowX Process Engine, Data Search, Audit Core).
