...
In version 4.1, you need to configure Redis using the following default properties with the following default corresponding environment variables:
...
For managing microservices metrics, especially exporting metrics to Prometheus, the following environment variable is used:
...
For OAuth2 resource server configuration, especially when using an opaque token, the following setup is provided, the environment variables involved are:
...
For microservices that use elastic search:
...
* `SPRING_DATASOURCE_ENGINE_JDBC_URL`: URL of the engine database.
* `SPRING_DATASOURCE_ENGINE_USERNAME`: Username for the engine database.
* `SPRING_DATASOURCE_ENGINE_PASSWORD`: Password for the engine database.


#### License database source

* `SPRING_DATASOURCE_LICENSE_JDBC_URL`: URL of the license database.
* `SPRING_DATASOURCE_LICENSE_USERNAME`: Username for the license database.
* `SPRING_DATASOURCE_LICENSE_PASSWORD`: Password for the license database.
