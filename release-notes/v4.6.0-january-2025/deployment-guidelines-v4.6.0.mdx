...
In the 4.6.0 release, a new configuration option has been added to the Integration Designer for enhanced Kafka topic management.
...
1. **Kafka Consumer Groups and Threads**:
   - Added environment variables for consumer groups:
...
2. **Kafka Payload Size Limit**:
   - New configuration: `FLOWX_KAFKA_PAYLOAD_SIZE_LIMIT: 512000`.

3. **Scheduler Enhancements**:
   - Retry failed deployments every minute: 
      - `FLOWX_SCHEDULER_RETRY_FAILED_DEPLOYMENTS_CRON: "0 * * * * *"`
   - Enabled master election:
     - `FLOWX_SCHEDULER_MASTER_ELECTION_ENABLED: true`
     - `FLOWX_SCHEDULER_MASTER_ELECTION_CRON_EXPRESSION: "*/30 * * * * *"`

4. **Kafka Topic Updates**:
   - Build resource topics:
     - **Create**: `${kafka.topic.naming.prefix}build${dot}create${kafka.topic.naming.suffix}`.
     - **Update**: `${kafka.topic.naming.prefix}build${dot}update${kafka.topic.naming.suffix}`.
