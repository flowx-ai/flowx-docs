...
Besides adding a new column, there is no other significant impact on the DB schema. The partitioning process introduces a new column, but there are no other significant changes to the schema.
