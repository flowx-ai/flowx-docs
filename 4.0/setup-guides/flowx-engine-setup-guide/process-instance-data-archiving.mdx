...
Partitions can be created per day, week or month. This means that a partition ID is computed at injection time for each row of `process_instance` and related tables.
...
Archiving involves moving older or less frequently accessed data from the primary tables to separate archive tables. This helps in reducing the load on the primary tables, thus improving performance and manageability.
...
Converting the detached partition into a new table named `process_instance_<partition_id>`.
</Step>
<Step title ="Steps">
...
## Partitioning and archiving in PostgreSQL
...
PostgreSQL supports table partitioning, allowing large tables to be divided into smaller, more manageable pieces.

</Step>

<Step title= "Implementation">

Partitioning in PostgreSQL is similar to Oracle, where tables can be divided based on specific criteria like date. For instance, a `process_instance` table can be partitioned by day, week, or month. 

<Info>
This process introduces a new column, but there are no other significant changes to the schema.
</Info>

</Step>
<Step title ="Benefits">
Improved performance for queries and maintenance operations.

Simplified management of large datasets.
</Step>
</Steps>
...
Create a new table named `process_instance_<partition_id>`.

<Tip>
The new table is created (same name format as for oracle: `process_instance_<partition_id>`) and data is moved from the primary table here. 
</Tip>
...
<Tip>
When starting the aversion of the process-engine, we recommend manually executing the setup SQL commands from Liquibase, as they may take more time. After setup, all existing information will go into the initial partition.
</Tip>
