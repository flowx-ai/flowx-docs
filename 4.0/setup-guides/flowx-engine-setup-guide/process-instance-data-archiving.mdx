...
- [**Partitioned tables**](https://docs.oracle.com/en/database/oracle/oracle-database/18/vldbg/partition-concepts.html#GUID-6D369646-16AF-487B-BF32-5F6569D27C8A) are utilized,llowing for efficient data management and query performance. 
- Archiving means detaching the specific partition from the main table and converting it into a new table, named `process_instance_<partition_id>`.

### PostgreSQL partitioning 

- Adding a new column does not significantly impact the overall database schema.

- The new table is created (same name format as for oracle: `process_instance_<partition_id>`) and data is moved from the primary table here. Since here the DB has more work to do than just changing some labels (actual data insert and delete vs relabel a partition into a table, for oracle) the data move is batched and the batch size is configurable.
