...
- To do so, go to **Process Definitions -> Your process definition -> Process Settings -> General -> Expiry time**:
...
This section contains environment variables that control the settings for data partitioning and archiving r the archiving scheduler. These settings determine how data is partitioned, retained, and managed, including compression and batch processing.
...
When partitioning is enabled, the Elasticsear indexing strategy must also be enabled and configured on FlowX Engine setup.
