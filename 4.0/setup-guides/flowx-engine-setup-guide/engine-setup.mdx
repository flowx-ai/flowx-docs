...
* **Ensure Process Termination**: Make sure your process instances are terminated. This means that no process should remain unfinished for any reason.
* **Set Process Expiry**n: Set an expiry on your processes. This ensures that your processes will reach a FINISHED status, either naturally or through expiration.
</Info>


<Steps>
<Step title="Setup">
    - Execute SQL setup commands manually.

<Check>
On the Production environment, before starting the new version of the process-engine, we recommend manually executing the setup SQL commands from Liquibase, as they may take more time. After setup, all existing information will go into the initial partition. The initial partition will continue to be used until a new partition_id is generated according to the partitioning interval (DAY, WEEK, MONTH). Future partitions will be created automatically.
</Check>
    
 - Install the latest build version of `process-engine`.
...
- Define the appropriate cron expression for the scheduler.
...
<Info>
Compression for detached partitions is available only for Oracle DBs.
</Info>
