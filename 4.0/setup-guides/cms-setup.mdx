...
### Configuring the maximum size of the files

To configure the limit of the maximum size of the files uploaded through the CMS services (like for example, in the Media Library) you can change the values of the following environment variables:

* `SPRING_SERVLET_MULTIPART_MAX_FILE_SIZE: ${MULTIPART_MAX_FILE_SIZE:50MB}` (default : 50 MB): Maximum file size allowed for uploads.
* `SPRING_SERVLET_MULTIPART_MAX_REQUEST_SIZE: ${MULTIPART_MAX_REQUEST_SIZE:50MB}` (default: 50 MB): Maximum request size allowed for uploads.