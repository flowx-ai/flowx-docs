---
title: Reporting
description: The Reporting setup guide assists in configuring the reporting plugin, relying on specific dependencies and configurations.
icon: chart-simple
---
...
- **Reporting-plugin Helm Chart**:
  ...
  - Utilizes Spark Operator (more info [**here**](https://github.com/GoogleCloudPlatform/spark-on-k8s-operator/blob/master/4.0/docs/quick-start-guide.md)).
- **Superset**:
...
5. Configure the correct database ENV variables in the `spark-app.yml` file (check them in the below examples with/without webhook).
