...
Each of these steps is covered in this guide. If you need further information or a broader understanding of Keycloak, refer to the official Keycloak documentation:
...
Use the appropriate URL for your environment (e.g., QA, development, production).
...
In the top left corner dropdown menu, click **Create Realm**.
...
You can either create or import user groups into a realm. For importing, use the provided script to import a super admin group with the necessary default user roles.
...
In the **Groups** section, search for a group, in our case: `FLOWX_SUPER_USERS`.
<Frame>
...
<Step title="Set Valid Redirect URIs">
**Valid Redirect URIs**: Populate this field with the appropriate URIs. This step is mandatory.
...
A service account grants direct access to the Keycloak API for a specific component. 

Each client can have a built-in service account that allows it to obtain an access token.
