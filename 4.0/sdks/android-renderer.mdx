...
dependencies {
    ...
    implementation("ai.flowx.android:android-sdk:3.0.3")
    ...
}
```
...
Before actually using the SDK, it is recommended to check the compatibility between the renderer and the currently deployed FlowX services.<br/><br/>
...
interface CustomStepperHeaderData {
    // percentage of total progress (e.g. 0.25f) for the current main step
    val mainStepsProgress: Float
    // title for the current main step; it contains the empty value ("") if not defined
    val stepTitle: String
    // percentage of total progress (e.g. 0.18f) for the current substep;
    // it has the same value as `mainStepsProgress` when there are no substeps defined
    val subStepsProgress: Float
    ...
    // index of the current main step
    val stepNumber: Int
