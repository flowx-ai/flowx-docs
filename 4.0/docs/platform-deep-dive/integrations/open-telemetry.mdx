...
<Tip>
This feature is available starting with v4.1.0 platform release.
</Tip>

  <a href="https://docs.flowx.ai/release-notes/v4.1.0-may-2024/v4.1.0-may-2024" class="label-version" target="_blank" rel="noopener noreferrer">
    v4.1.0
  </a>
...
<Info>
Tracing with Open Telemetry is available starting with FlowX.AI v.4.1 release.
</Info>
...
* **Query Language**: Utilize Grafana's query language to create complex search conditions combining multiple attributes
* **Search Strategies**:
  * **Process-based**: Search by `fx.processName` to track specific business processes
  * **Instance-based**: Use `fx.processInstanceUuid` to follow a specific process instance
  * **Action-based**: Filter by `fx.actionName` to monitor specific actions
  * **Node-based**: Search by `fx.nodeName` or `fx.nodeId` to analyze particular workflow steps
* **Time-based Search**: Combine attribute filters with time ranges to narrow down specific incidents
* **Error Analysis**: Filter by error status and combine with other attributes to identify patterns in failures
* **Custom Dashboards**: Create saved searches and custom dashboards for frequently used queries

<Tip>
For optimal search performance, index the most frequently used search attributes in your tracing backend.
</Tip>
