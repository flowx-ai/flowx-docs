---
title: FlowX.AI Audit
description: The Audit service provides a centralized location for all audit events. The following details are available for each event.
---
...
The Audit service provides comprehensive search and filtering capabilities to help users quickly locate specific audit events.

### Date Range Filtering

* Filter events by selecting a specific date range using the date picker
* Pre-defined quick filters available: Today, Last 7 days, Last 30 days
* Custom date range selection with start and end dates
* Time precision up to minutes and seconds
...
* **User Search**:
  ...
  * Matches partial usernames and system identifiers
  * Case-insensitive search

* **Subject Search**:
  * Single selection from predefined list
  * Quick filtering through type-ahead functionality
  * Hierarchical categorization of subjects
  * Clear visual indicators for selected values

* **Subject Identifier Search**:
  * Exact match search
  * Supports copying and pasting identifiers
  * Clear button to reset the search
  * Validation for proper identifier format
...
* Sort capability by timestamp (ascending/descending)
* Export filtered results functionality
* Counter showing total matching results

### Search Tips

* Combine multiple filters for more precise results
* Use the date range filter first to narrow down the time period
* Clear individual filters by clicking the 'x' button
