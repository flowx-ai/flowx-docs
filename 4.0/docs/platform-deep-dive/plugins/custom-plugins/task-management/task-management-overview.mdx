...
* **Search**: An advanced search functionality that allows you to:
  * Search by process keys stored in the process
  * Use filters to narrow down results
  * Search across multiple fields including title, assignee, and status
  * Perform partial text matches
  * Save frequently used search criteria
  * Export search results
  * Use boolean operators (AND, OR) for complex searches
  * Clear search history and filters
