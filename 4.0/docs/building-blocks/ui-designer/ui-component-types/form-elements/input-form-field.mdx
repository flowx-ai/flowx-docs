...
* **Process data key**: Establishes the binding between the form element and process data, enabling its later use in [decisions](../../../node/exclusive-gateway-node), [business rules](../../../actions/business-rule-action/business-rule-action) or [integrations](../../../node/message-send-received-task-node#from-integration).
...
* **Helper Text**: Additional information about the input field, which can be optionally hidden within an infopoint.
...
* **Hide Condition**: A JavaScript expression used to hide the input field when it is evaluated to your desired result. 
...
<Info>
In the example above, we used a rule to hide an input field until another one is filled. The "Mortgage" input field, which remains hidden until users fill in the "Income" field.
</Info>
...
* **Disabled condition** - JavaScript expression used to disable the Input Field when it returns a truthy value
...
* **Height**: Determines the vertical size of the input box on the screen.
