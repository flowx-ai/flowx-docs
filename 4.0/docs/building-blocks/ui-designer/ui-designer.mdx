...
Different UI component types can be configured using UI Designer. The components can be configured only using [user task](/4.0/docs/building-blocks/node/user-task-node) nodes or [milestone](../../node/milestone-node) nodes.
