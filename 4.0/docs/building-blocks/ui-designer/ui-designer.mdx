...
#### Transition from Process Designer

* Upon navigating from the **Process Designer** to the **UI Designer**, the **Settings** and **Generic** tabs are displayed.
If the transition is from the **Web Tab Navigation Areas**, the web preview is shown; if from the **Mobile Tab Navigation Areas**, the iOS preview is displayed.

#### Preview Display

* Platform-specific previews (Web/iOS/Android) are shown in their respective tabs.
* The **Generic tab** displays the preview of the last visited platform, including any overrides set on that platform.
* Transitioning from the **Styles** tab maintains the current preview and switches to the **Generic tab**.

<Info>
**Overrides**: If a property has an override on the last selected platform, it is reflected in the preview displayed in the **Generic tab**.
</Info>
...
Different UI component types can be configured using UI Designer. The components can be configured only using [user task](/4.0/docs/building-blocks/node/user-task-node) nodes or [milestone](/4.0/docs/building-blocks/node/milestone-node) nodes.
...
* [Form](/4.0/docs/building-blocks/ui-designer/ui-component-types/form-elements)
...
For more information about Milestone node UI components, click [here](/4.0/docs/building-blocks/node/milestone-node).
