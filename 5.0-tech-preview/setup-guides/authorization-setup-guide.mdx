...
description: Setup and configuration guide for the CAS microservice (authorization-system) that manages workspace permissions, user access control, and ACL rules in FlowX 5.0
icon: shield-check
---
...
- **admin service**: REST API communication with authorization-system via CAS client library
...
<Steps>
  ...
  <Step title="Deploy authorization-system Infrastructure">
...
<AccordionGroup>
  <Accordion title="CAS Connection Issues" icon="plug">
    ...
    **Diagnosis**:
    ...
    curl -f http://authorization-system-service:8080/actuator/health
    
    # Check CAS logs
    docker logs authorization-system-service
    kubectl logs -l app=authorization-system -n flowx
    ```
    
    **Solutions**:
    - Verify authorization-system service is running and healthy
  ...
  <Accordion title="SpiceDB Permission Issues" icon="ban">
    ...
    **Diagnosis**:
    ...
    curl http://authorization-system-service:8080/actuator/health/spicedb
  ...
  <Accordion title="Database Migration Errors" icon="database">
    ...
    **Solutions**:
    - Ensure authorization-system has exclusive write access to CAS database
  ...
  <Accordion title="Service Communication Errors" icon="network-wired">
    **Symptoms**: Services cannot communicate with authorization-system or SpiceDB
