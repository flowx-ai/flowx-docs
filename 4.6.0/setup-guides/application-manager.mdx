...
* **Purpose**: Configures a cron job to retry failed deployments every minute.
...
- **Consumer Groups**:
  - `kafka.consumer.group-id.build.create`: `buildCreateGroup`
  - `kafka.consumer.group-id.build.update`: `buildUpdateGroup`

- **Consumer Threads**:
  - `kafka.consumer.threads.build.create`: `2`
  - `kafka.consumer.threads.build.update`: `4`
