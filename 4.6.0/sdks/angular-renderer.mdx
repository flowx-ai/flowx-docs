...
```bash
npm install \
  @flowx/core-sdk@5.53.4 \
  @flowx/core-theme@5.53.4 \
  @flowx/angular-sdk@5.53.4 \
  @flowx/angular-theme@5.53.4 \
  @flowx/angular-ui-toolkit@5.53.4 \
  @angular/cdk@19 \
...
### Custom Nomenclator service

The SDK allows you to create a custom nomenclator service that can be used to fetch enumeration data from an external source. These custom nomenclator services should implement the `NomenclatorServiceInterface`.

* Here is an example of how to create a custom nomenclator service that uses the `HttpClient` to fetch data from an external API:
...
// Add the interceptor to the providers array in the main app module
{
  ...
  useFactory: (http: HttpClient) => ({ 
    get: (name: string) => lastValueFrom(http.get(`https://some-external-api.com?name=${name}`)),
  }),
  deps: [HttpClient],
},
...
| appId            | The application id                     | string | -          | true      |                              |
| viewDefinitionId |                                        | string | -          | true      |                              |
