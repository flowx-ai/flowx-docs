    ...
    private fun initFlowXSdk() {
        FlowxSdkApi.getInstance().init(
            ...
            onNewProcessStarted = { processInstanceUuid ->
                // Send a local broadcast message to notify the Activity currently displaying the running process.
...
One way to handle this is to send a local broadcast message to notify the Activity currently displaying the running process.
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            LifecycleStartEffect(true) {
                LocalBroadcastManager.getInstance(context.applicationContext).registerReceiver(yourBroadcastReceiver, IntentFilter("some.intent.filter.indentifier"))
                onStopOrDispose {
                    runCatching {
                        LocalBroadcastManager.getInstance(context.applicationContext).unregisterReceiver(yourBroadcastReceiver)
                    }
                }
            }
