...
<Info>
Support for legacy Python 2.7 via Jython can be enabled with an environment variable.
</Info>

<Info>
**New in v4.7.1**: Python has been upgraded from 2.7.0 (Jython) to 3.11.7 (GraalPy), and JavaScript has been upgraded from ECMAScript 5.1 (Nashorn) to ECMAScript 15 (GraalJS). These updates provide better performance and access to modern language features.
</Info>
...
<Info>
Jython support is considered legacy and is only enabled via environment variables. We recommend using Python 3 via GraalPy for all new development.
</Info>
