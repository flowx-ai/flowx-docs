...
<Tip> 
We've created a comprehensive FlowX.AI Academy course guiding you through the process of **Creating a Document Template in Designer**. Access the course [**here**](https://academy.flowx.ai/module/create-document-templates-flowxai-designer) for detailed instructions and insights.
</Tip>
  ...
  "documentList": [
    {
     ...
     "includeBarcode": false //if you want to include a barcode, set it to true
    }
  ]
}
```
...
5. Configure the third node (Receive Message Task): 
    ...
    *  Add the topic where the engine will wait for the response from the plugin; in our example `ai.flowx.updates.document.html.generate.v1` and its key: `generatedDocuments`
...
TODO Dragos: duplicate of the section above?
