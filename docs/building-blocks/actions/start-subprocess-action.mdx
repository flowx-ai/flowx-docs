---
title: Start subprocess action
description: A **Start subprocess action** is an **action** that allows you to start a **subprocess** from another (parent) **process**.
---
...
To use a process as a [subprocess](../process/subprocess) you must first create it. Once the subprocess is created, you can start it from another (parent) process. To do this, you will need to add a **Start Subprocess** action to a [**User task**](../node/task-node.md) node in the parent process or by using a [subprocess run node](../node/subprocess-run-node.md).
